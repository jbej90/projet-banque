/**
 * Develop by http://www.eyecon.ro/datepicker/
 *
 * Customized by Damien Villeneuve
 *  - add highlight class on the current month
 *  - add active class on the selected month
 */
...
(function($) {
	...
	$.extend(Monthpicker.prototype, {
		...
		/**
		 * Method customized by Damien Villeneuve
		 */
		_generateHTML: function(inst) {
			var tbody = $('<tbody/>');
			var tr, td, currentMonthDate;
			
			var minDate = $.datepicker._getMinMaxDate(inst, 'min');
			var maxDate = $.datepicker._getMinMaxDate(inst, 'max');
			var _defaultDate = $.datepicker._getDefaultDate(inst);
			var defaultDate = new Date(_defaultDate.getFullYear(), _defaultDate.getMonth(), 1);
			var currentDate = $.datepicker._daylightSavingAdjust((!inst.currentDay ? new Date(9999, 9, 9) : new Date(inst.currentYear, inst.currentMonth, inst.currentDay)));
			...
			for(var row = 0; row < 4; row++) {
				...
				for(var cell = row*3; cell < row*3+3; cell++) {
					currentMonthDate = new Date(inst.selectedYear, cell, 1);
					
					// check if month is selectedable
					if(currentMonthDate >= maxDate || currentMonthDate <= minDate) {
						td = $('<td/>', {'class': 'ui-datepicker-unselectable ui-state-disabled'});
						// create a link and append to td
						$('<span/>', {'style': 'text-align: center; display:block; line-height: 2em;','class': 'ui-state-default'}).html($.datepicker._defaults.monthNamesShort[cell]).appendTo(td);
					}
					else
					{
						td = $('<td/>', {'style': 'text-align:center'});
						// create a link and append to td
						$('<a/>', {'style': 'text-align:center; display:block; line-height: 2em;', 'href': '#', 'class': 'ui-state-default'}).html($.datepicker._defaults.monthNamesShort[cell]).appendTo(td);
					...
					// Check for the actual month
					if (currentMonthDate.getTime() == defaultDate.getTime()) {
